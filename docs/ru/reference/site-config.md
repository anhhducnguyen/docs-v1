---
description: Полный справочник по параметрам конфигурации сайта , включая настройки приложения, темы и сборки.
outline: deep
---

# Конфигурация сайта {#site-config}

Конфигурация сайта — это место, где вы можете определить глобальные настройки сайта. Параметры конфигурации приложения определяют настройки, которые применяются к каждому сайту , независимо от того, какая тема на нем используется. Например, базовый каталог или название сайта.

## Обзор {#overview}

### Разрешение конфигурации {#config-resolution}

Конфигурация всегда считывается из файла `<root>/.vitepress/config.[ext]`, где `<root>` — это корень вашего [проекта](../guide/routing#root-and-source-directory) , а `[ext]` — одно из поддерживаемых расширений файла. TypeScript поддерживается из коробки. Поддерживаемые расширения включают `.js`, `.ts`, `.mjs` и `.mts`.
...
- **Vite**
  ...
  Вы можете настроить базовый экземпляр Vite с помощью опции [vite](#vite) в конфигурации . Нет необходимости создавать отдельный файл конфигурации Vite.

- **Vue**

   уже включает в себя официальный плагин Vue для Vite ([@vitejs/plugin-vue](https://github.com/vitejs/vite-plugin-vue)). Вы можете настроить его параметры с помощью опции [vue](#vue) в конфигурации .

- **Markdown**

  Вы можете настроить базовый экземпляр [Markdown-It](https://github.com/markdown-it/markdown-it) с помощью опции [markdown](#markdown) в конфигурации .
...
Дополнительные элементы для отображения в теге `<head>` в HTML-странице. Добавленные пользователем теги выводятся перед закрывающим тегом `head`, после тегов .
...
Если установить значение `true`,  будет удалять из URL-адресов завершающий `.html`. Также смотрите [Создание чистых URL-адресов](../guide/routing#generating-clean-urls).
...
Если установлено значение `true`,  не будет завершать сборку из-за неработающих ссылок.
...
Настройте параметры парсера Markdown.  использует [Markdown-it](https://github.com/markdown-it/markdown-it) в качестве парсера и [Shiki](https://github.com/shikijs/shiki) для подсветки синтаксиса языка. Внутри этой опции вы можете передать различные параметры, связанные с Markdown, в соответствии с вашими потребностями.
...
Хуки для сборки  позволяют добавлять на сайт новую функциональность и поведение:
...
`buildEnd` — это хук CLI сборки, который будет запущен после завершения сборки (SSG), но до выхода из процесса  CLI.
...
`transformHead` — это хук сборки для преобразования заголовка перед генерацией каждой страницы. Это позволит вам добавить в конфигурацию  записи, которые не могут быть добавлены статически. Вам нужно только вернуть дополнительные записи, они будут автоматически объединены с существующими.
