---
description: Узнайте, как ссылаться на статические ресурсы, такие как изображения, медиафайлы и шрифты, и обрабатывать их в .
---
...
В этом случае рекомендуется обернуть путь с помощью [хелпера `withBase`](../reference/runtime-api#withbase), предоставляемого :
