---
description: Настройте интернационализацию (i18n) в  для поддержки нескольких языков на вашем сайте.
---
...
Однако по умолчанию  не будет перенаправлять `/` на `/en/`. Для этого вам нужно будет настроить свой сервер. Например, в Netlify вы можете добавить файл `docs/public/_redirects` следующим образом:
