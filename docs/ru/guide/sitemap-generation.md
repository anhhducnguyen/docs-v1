---
description: Сгенерируйте файл sitemap.xml для вашего сайта , чтобы улучшить индексацию в поисковых системах.
---

# Генерация карты сайта {#sitemap-generation}

 поставляется с готовой поддержкой генерации файла `sitemap.xml` для вашего сайта. Чтобы включить её, добавьте следующее в файл `.vitepress/config.js`:
