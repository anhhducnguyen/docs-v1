---
description: Создайте и используйте пользовательскую тему в  для полного контроля над внешним видом вашего сайта.
---
...
```
 ...
 всегда будет использовать пользовательскую тему вместо темы по умолчанию, если обнаружит наличие входного файла темы. Однако вы можете [расширить тему по умолчанию](./extending-default-theme), чтобы выполнить расширенные настройки поверх нее.

## Интерфейс темы {#theme-interface}

Пользовательская тема  определяется как объект со следующим интерфейсом:
...
interface EnhanceAppContext {
  ...
  router: Router // Экземпляр маршрутизатора 
...
//  предварительно настроен с помощью @vitejs/plugin-vue.
...
Экспорт по умолчанию является единственным контрактом для пользовательской темы, и только свойство `Layout` является обязательным. Таким образом, технически тема  может быть простой, как один компонент Vue.
...
3. Если ваша тема требует настройки конфигурации , экспортируйте эту конфигурацию в подпапку пакета (например. `my-theme/config`), чтобы пользователь мог расширить её.
...
Если тема требует специальных настроек , вам нужно будет также расширить их в своем собственном конфиге:
