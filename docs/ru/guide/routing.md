---
description: Изучите файловую маршрутизацию , динамические маршруты, чистые URL-адреса и перезапись путей.
outline: deep
---
...
## Маршрутизация на основе файлов {#file-based-routing}
 ...
 использует маршрутизацию на основе файлов, что означает, что сгенерированные HTML-страницы отображаются на основе структуры папок исходных файлов Markdown. Например, при следующей структуре папок:
...
В файловой структуре проекта  есть два важных понятия: **корень проекта** и **директория с исходными файлами**.

### Корень проекта {#project-root}

Корень проекта — это место, где  будет пытаться искать специальный каталог `.vitepress`. Директория `.vitepress` — это зарезервированное место для конфигурационного файла , кэша dev-сервера, результатов сборки и дополнительного кода настройки темы.

Когда вы запускаете `vitepress dev` или `vitepress build` из командной строки,  будет использовать текущий рабочий каталог в качестве корня проекта. Чтобы указать подкаталог в качестве корневого, нужно передать команде относительный путь. Например, если ваш проект  находится в папке `./docs`, вам следует выполнить команду `vitepress dev docs`:
...
При создании ссылок между страницами можно использовать как абсолютные, так и относительные пути. Обратите внимание, что хотя расширения `.md` и `.html` будут работать, лучше всего опускать расширения файлов, чтобы  мог генерировать конечные URL на основе вашего конфига.
...
### Ссылки на страницы, не принадлежащие  {#linking-to-non-vitepress-pages}

Если вы хотите создать ссылку на страницу вашего сайта, которая не создана , вам нужно будет либо использовать полный URL-адрес (откроется в новой вкладке), либо явно указать цель:
...
Для обслуживания чистых URL-адресов с помощью  требуется поддержка на стороне сервера.
:::

По умолчанию  разрешает входящие ссылки на URL-адреса, заканчивающиеся на `.html`. Однако некоторые пользователи могут предпочесть «Чистые URL-адреса» без расширения `.html` — например, `example.com/path` вместо `example.com/path.html`.
...
Если эта функция вам доступна, вы также можете включить собственную опцию конфигурации  [`cleanUrls`](../reference/site-config#cleanurls), чтобы обеспечить следующее поведение:
...
И вы хотите, чтобы страницы  генерировались следующим образом:
...
Поскольку  — это генератор статических сайтов, возможные пути страниц должны быть определены во время сборки. Поэтому динамическая маршрутная страница **должна** сопровождаться **файлом загрузчика путей**. Для `packages/[pkg].md` нам понадобится `packages/[pkg].paths.js` (`.ts` также поддерживается):
