---
description: Убедитесь, что компоненты вашей темы  и пользовательский код совместимы с серверным рендерингом.
outline: deep
---

# Совместимость с SSR {#ssr-compatibility}

 предварительно рендерит приложение в Node.js во время производственной сборки, используя возможности Vue по рендерингу на стороне сервера (SSR). Это означает, что весь пользовательский код в компонентах темы подлежит проверке на совместимость с SSR.
...
### `defineClientComponent` {#defineclientcomponent}
 ...
 предоставляет удобный помощник для импорта компонентов Vue, которые получают доступ к API браузера при импорте.
