---
description: 在  中创建和使用自定义主题，全面控制站点的外观和风格。
---
...
当检测到存在主题入口文件时， 总会使用自定义主题而不是默认主题。但你可以[扩展默认主题](./extending-default-theme)来在其基础上实现更高级的自定义。

## 主题接口 {#theme-interface}

 自定义主题是一个对象，该对象具有如下接口：
...
默认导出是自定义主题的唯一方式，并且只有 `Layout` 属性是必须的。所以从技术上讲，一个  主题可以是一个单独的 Vue 组件。
...
3. 如果主题需要调整  配置，请在包的子路径下 (例如 `my-theme/config`) 下导出该配置，以便用户扩展。
