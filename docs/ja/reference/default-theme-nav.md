---
description: デフォルトテーマのナビゲーションバーにサイトタイトル、ロゴ、メニューリンクを設定する方法。
---
...
既定では、リンクが外部かどうかに応じて  が `target` と `rel` を自動設定します。必要であれば明示的に指定することもできます。
...
コンポーネントはナビゲーションバー内にレンダリングされます。 は次の追加 props をコンポーネントに提供します。
