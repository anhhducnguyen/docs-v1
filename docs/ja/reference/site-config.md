---
outline: deep
description: のサイト設定オプションの完全なリファレンス。アプリレベルの設定、テーマ、ビルドオプションを含みます。
---

# サイト設定 {#site-config}

サイト設定では、サイト全体のグローバル設定を定義します。アプリ設定オプションは、使用するテーマに関係なく、すべての  サイトに適用されます。たとえば、ベースディレクトリやサイトのタイトルなどです。

## 概要 {#overview}

### 設定ファイルの解決 {#config-resolution}

設定ファイルは常に `<root>/.vitepress/config.[ext]` から解決されます。`<root>` は  の[プロジェクトルート](../guide/routing#root-and-source-directory)で、`[ext]` にはサポートされる拡張子が入ります。TypeScript はそのまま使えます。サポートされる拡張子は `.js`、`.ts`、`.mjs`、`.mts` です。
...
- **Vite**
  ...
  Vite の設定は  設定の [vite](#vite) オプションで行えます。別途 Vite の設定ファイルを作る必要はありません。

- **Vue**

   には公式の Vue プラグイン（[@vitejs/plugin-vue](https://github.com/vitejs/vite-plugin-vue)）が同梱されています。オプションは  設定の [vue](#vue) から指定できます。

- **Markdown**

  既定の [Markdown-It](https://github.com/markdown-it/markdown-it) インスタンスは、 設定の [markdown](#markdown) オプションでカスタマイズできます。
...
ページ HTML の `<head>` に追加で出力する要素。ユーザーが追加したタグは、 のタグの後、`</head>` の直前にレンダリングされます。
...
Markdown パーサの設定。 はパーサに [Markdown-it](https://github.com/markdown-it/markdown-it)、構文ハイライトに [Shiki](https://github.com/shikijs/shiki) を使用しています。必要に応じて Markdown 関連の各種オプションを指定できます。
...
## ビルドフック {#build-hooks}
 ...
 のビルドフックを使うと、サイトに機能や振る舞いを追加できます。
...
`buildEnd` はビルド CLI フックです。ビルド（SSG）が完了した後、 CLI プロセスが終了する前に実行されます。
