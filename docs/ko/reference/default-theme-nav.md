---
description: 사이트 제목, 로고 및 메뉴 링크를 포함한  기본 테마의 네비게이션 바를 구성하세요.
---
...
기본적으로 는 링크가 외부 링크인지 여부에 따라 `target`과 `rel` 어트리뷰트를 자동으로 결정합니다. 하지만 원한다면 이를 커스터마이징 할 수도 있습니다.
...
컴포넌트는 네비게이션 바에 렌더링될 것입니다. 는 컴포넌트에 다음과 같은 추가 프로퍼티를 제공합니다:
