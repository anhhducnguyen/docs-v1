---
description: 에서 국제화(i18n)를 설정하여 사이트에서 여러 언어를 지원하세요.
---
...
그러나 기본적으로 는 `/`를 `/en/`로 리다이렉션하지 않습니다. 이를 위해 서버를 구성해야 합니다. 예를 들어, Netlify에서는 다음과 같이 `docs/public/_redirects` 파일을 추가할 수 있습니다:
