---
outline: deep
description: 의 파일 기반 라우팅, 동적 라우트, 클린 URL 및 경로 재작성을 이해하세요.
---
...
는 파일 기반 라우팅을 사용하므로, 생성된 HTML 페이지는 소스 마크다운 파일의 디렉토리 구조에서 매핑됩니다. 예를 들어, 다음과 같은 디렉토리 구조가 있다고 가정하면:
...
## 루트와 소스 디렉토리 {#root-and-source-directory}
 ...
 프로젝트의 파일 구조에는 두 가지 중요한 개념이 있습니다: **프로젝트 루트**와 **소스 디렉토리**입니다.

### 프로젝트 루트 {#project-root}

프로젝트 루트는 가 `.vitepress` 특수 디렉토리를 찾으려고 하는 위치입니다. `.vitepress` 디렉토리는 의 구성 파일, 개발 서버 캐시, 빌드 출력물, 그리고 선택적인 커스텀 테마 코드를 위한 예약된 위치입니다.

명령줄에서 `vitepress dev`나 `vitepress build`를 실행하면 는 현재 작업 디렉토리를 프로젝트 루트로 사용합니다. 서브 디렉토리를 루트로 지정하려면 명령어에 상대 경로를 전달해야 합니다. 예를 들어,  프로젝트가 `./docs`에 위치한 경우, `vitepress dev docs`를 실행해야 합니다:
...
페이지 간 링크를 만들 때 절대 경로와 상대 경로 모두 사용할 수 있습니다. `.md`와 `.html` 확장자 모두 작동하지만, 가 구성에 따라 최종 URL을 생성할 수 있도록 파일 확장자를 생략하는 것이 좋은 방법입니다.
...
###  페이지가 아닌 페이지로 연결 {#linking-to-non-vitepress-pages}

로 생성되지 않은 페이지에 연결하려면 전체 URL(새 탭에서 열림)을 사용하거나 명시적으로 target을 지정해야 합니다:
...
::: warning 서버 지원 필요
가 간결한 URL을 제공하려면 서버 측 지원이 필요합니다.
:::

기본적으로 는 `.html`로 끝나는 URL로 들어오는 링크를 처리합니다. 하지만 일부 사용자는 `.html` 확장자가 없는 "간결한 URL"을 선호할 수 있습니다. 예를 들어, `example.com/path.html` 대신 `example.com/path`.
...
이 기능을 사용할 수 있는 경우, 의 자체 [`cleanUrls`](../reference/site-config#cleanurls) 구성 옵션을 활성화해서:
...
그리고  페이지를 다음과 같이 생성하고 싶다고 가정해 봅시다:
...
는 정적 사이트 생성기이므로, 가능한 페이지 경로는 빌드 시에 결정되어야 합니다. 따라서 동적 라우트 페이지는 반드시 **경로 로더 파일**과 함께 제공되어야 합니다. `packages/[pkg].md`의 경우, `packages/[pkg].paths.js` (`.ts`도 지원됩니다) 파일이 필요합니다:
