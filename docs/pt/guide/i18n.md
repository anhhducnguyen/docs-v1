---
description: Configure a internacionalização (i18n) no  para oferecer suporte a múltiplos idiomas no seu site.
---
...
No entanto,  não redirecionará `/` para `/en/` por padrão. Você precisará configurar seu servidor para isso. Por exemplo, no Netlify, você pode adicionar um arquivo `docs/public/_redirects` assim:
