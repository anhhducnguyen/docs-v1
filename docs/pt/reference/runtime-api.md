---
description: Referência das APIs de tempo de execução do , incluindo composables, funções auxiliares e componentes integrados.
---

# API em Tempo de Execução {#runtime-api}

 oferece várias APIs embutidas para permitir o acesso aos dados da aplicação.  vem também com alguns componentes embutidos que podem ser usados globalmente.
...
Retorna a instância do roteador  para que você possa navegar programaticamente para outra página.
...
Como as aplicações  são interpretadas no lado do servidor em Node.js ao gerar builds estáticos, qualquer uso do Vue deve seguir os requisitos de código universal. Em resumo, certifique-se de acessar apenas APIs do Navegador / DOM em ganchos `beforeMount` ou `mounted`.
